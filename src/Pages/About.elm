...
viewPlaceholder model =
    section [ class "bg-surface-2 p-8 rounded-xl w-[clamp(18rem,28vw,30rem)] md:scale-125", ariaLabelledby "heading" ]
        ...
        , div [ class "relative" ]
            ...
            , p [ class "absolute left-2 top-1/2 -translate-y-1/2 text-xs text-[#ffffff86] pointer-events-none" ] [ text "Input value: " ]
            ]
        , div [ class "grid grid-cols-[max-content,1fr] items-center bg-surface-1 p-2 rounded-lg mb-5 w-full h-8" ]
            [ em [ class "text-xs text-[#ffffff86] pr-2" ] [ text "Inverted value: " ]
        ...
        , div [ class "flex items-center justify-center gap-5" ]
            ...
            , p [ class "bg-surface-1 px-6 py-2 rounded-lg w-full text-center" ] [ text <| String.fromInt model.intValue ]
